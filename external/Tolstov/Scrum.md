---
created_at: 2025-10-17
updated_at: 2025-10-29
aliases: []
author: "[[Толстов.М.В]]"
title: Scrum
---
up:: [[Agile]]
source:: [Scrum \| Atlassian](https://www.atlassian.com/ru/agile/scrum)

**Scrum** — популярный фреймворк для "гибкой" разработки. Существуют и другие, см [[Agile]].

### Основные элементы
**Принцип:** работа итерациями ([[Scrum#Спринт|спринтами]]) с быстрой обратной связью и постоянным улучшением. В конце каждого спринта получаем обновленную, *работающую* версию продукта (пусть и изменения небольшие).

#### Роли
Product Owner — отвечает за продукт, приоритеты и требования.
Scrum Master — следит за процессом, убирает препятствия.
[[Scrum#Команда|Команда разработки]] — кросс-функциональная, выполняет работу.

#### Артефакты
- Product Backlog — список всех требований и задач.
- Sprint Backlog — задачи, выбранные на текущий спринт.
- Increment — готовый рабочий результат после спринта.

#### Церемонии
- Sprint Planning — планирование спринта.
- Daily Scrum — ежедневная короткая встреча команды.
- Sprint Review — демонстрация результатов спринта.
- Sprint Retrospective — анализ работы и улучшение процесса.


### Подробнее
#### Команда
Идеальная команда для этого метода управления — 5–9 человек, которые могут распределять задачи между собой и могут уверенно с высокой точностью сказать, сколько времени и ресурсов нужно на задачу.

#### Спринт
Спринт — это 1–4-недельный отрезок времени с обязательными расписаниями внутри.

---
См. также
```dataview
list WHERE contains(file.outlinks, this.file.link)
AND !contains(this.file.outlinks, file.link)
```


---
---
// Тут было написано раньше//

---
---
~~Scrum — это способ работать над проектом по кусочкам, короткими циклами (спринтами), когда команда сама решает, как делать задачи, показывает результат каждые пару недель и постоянно улучшает процесс.~~

## Основные Идеи
- Долгосрочное планирование бессмысленно.
- Не нужно ждать конца проекта, чтобы его оценить.
- Нужно часто проверять состояние проекта критично.
- Делить реализацию проекта на [[Scrum#спринт|спринты]].

## Спринт
Спринт — это 1–4-недельный отрезок времени с обязательными расписаниями внутри.

### Расписание Совещаний
1. **Начало спринта**
    Определение целей спринта и составление плана на спринт.
2. **Ежедневники**/**дейлики**
    Обратная связь по статусам от работников каждый день.
3. **Ретроспектива**
    Совещание, на котором собирается информация: что удалось сделать, что нет и что помешало в процессе работы.

## Команда
Идеальная команда для этого метода управления — 5–9 человек, которые могут распределять задачи между собой и могут уверенно с высокой точностью сказать, сколько времени и ресурсов нужно на задачу.

## Роли в команде
- **Scrum-мастер** — человек, который организует встречи и следит за выполнением задач.
- **Работники** - люди которые работают над проектом
